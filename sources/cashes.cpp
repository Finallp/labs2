...
cash_test::cash_test(std::string travel_variant)
...
void cash_test::straight() {
  ...
  for (const size_t size : _buffer_size) {
    ...
    clock_t start = clock();
    for (size_t it = 0; it < iterations; it++) {  //чтение
...
void cash_test::output(std::ostream& os) {
  os << "Investigation:\n";
  os << "\ttravel variant: \"" << _travel_variant << "\"\n";
  os << "\texperiments: \n";
